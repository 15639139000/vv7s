$(function(){
	...
	function build_noticeBulletin_table(result)
	{
		...
		$.each(noticeBulletin, function(index, item){
			...
			var href = path+"/noticeBulletin/getNoticeBulletinById?nbId="+item.nbId;
			var title = $("<td></td>").append("<a href='"+href+"' style='color: dimgray'>"+item.nbTitle+"</a>");
	...
	$("#submit").click(function(){
		...
		nbContent = nbContent.replace(/\r\n/g, '<br/>'); //IE9、FF、chrome  
        nbContent = nbContent.replace(/\n/g, '<br/>'); //IE7-8  
		nbContent = nbContent.replace(/\s/g, '&nbsp;'); //空格处理  
		if(nbTitle == ""){
