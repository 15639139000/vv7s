$(function(){
	...
	getAllCompanyRules(1);
	
	$("#title").append("公司制度列表");
	$("#insert").append("发布公司制度");
	...
	function getAllCompanyRules(pageNo)
	{
		$.ajax({
			...
			success : function(data){
				...
				if(data.total > 15){
					build_companyRules_pagenav(data);
				}
	...
	function build_companyRules_table(result)
	{
		...
		var companyRules = result.list;
		$.each(companyRules, function(index, item){
			var id = $("<td></td>").append(item.crId);
			var href = path+"/companyRules/getCompanyRulesById?crId="+item.crId;
			var title = $("<td></td>").append("<a href='"+href+"' style='color: dimgray'>"+item.crTitle+"</a>");
	...
	//构建分页信息
	function build_companyRules_pageInfo(result)
	{
		$("#pageInfo").empty();
	...
	//构建分页条
	function build_companyRules_pagenav(result)
	{
		$("#pageNav").empty();
		...
		else
		{
			...
			preLi.click(function(){
				getAllCompanyRules(result.pageNum - 1);
			});
		}
		...
		else
		{
			...
			lastLi.click(function(){
				getAllCompanyRules(result.pages);
			});
		}
	...
	$(document).on('click', '#home table tbody td a', function(){
		...
		if(text == "编辑"){
			...
			$("#crContent").html(content);
			$("#crId").val(id);
			$("#pageInfo").empty();
			$("#title").empty().append("公司制度列表");
			$("#submit").val("修改");
		}else if(text == "撤销"){
			if(window.confirm("确认删除?")){
				$.ajax({
					url : path + '/companyRules/delCompanyRulesById',
					type : 'POST',
					data : {"crId" : id},
					success : function(data){
						if(data == 1){
							getAllCompanyRules(currentPageNo);
						}
					}
				});
			}
	...
	$("#submit").click(function(){
		...
		crContent = crContent.replace(/\n/g, '<br/>'); //IE7-8  
        crContent = crContent.replace(/\s/g, '&nbsp;'); //空格处理  
		if(crTitle == ""){
		...
		if(text == "提交"){
			$.ajax({
				url : path + '/companyRules/insertCompanyRules',
				type : 'POST',
				data : {"crTitle" : crTitle, "crContent" : crContent},
				success : function(data){
					if(data == 1){
						$("#list").css("display","block");
						$("#edit").css("display","none");
			...
			$.ajax({
				url : path + '/companyRules/editCompanyRulesById',
				type : 'POST',
				data : {"crId" : crId, "crTitle" : crTitle, "crContent" : crContent},
				success : function(data){
					if(data == 1){
						$("#list").css("display","block");
						$("#edit").css("display","none");
